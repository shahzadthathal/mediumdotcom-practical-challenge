...
<div class="bg-blue-darker p-2 shadow text-xl text-white mt-10">
    <h3 class="pl-2">Update article</h3>
</div>
...
<div class="flex flex-wrap m-5">
   ...
   <div class="flex flex-wrap -mx-3 mb-6">
    <div class="w-full px-3">
      ...
      <input class="appearance-none block w-full bg-grey-lighter text-grey-darker border border-grey-lighter rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-grey" name="title" id="grid-title" type="text" placeholder="My awesome article" value="{{ $item->title }}">
    </div>
  </div>
    ...
    <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
      ...
      <div class="relative">
        <select class="block appearance-none w-full bg-grey-lighter border border-grey-lighter text-grey-darker py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-grey" id="grid-tags" name="tags[]" multiple="multiple">
            @foreach($tags as $tag)
            <option value="{{$tag->name}}" {{ (in_array($tag->name, $savedTags) ? 'selected' : '') }}>{{$tag->name}}</option>
            @endforeach
         </select>
  ...
  <div class="flex flex-wrap -mx-3 mb-6">
    <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
      ...
      <input class="appearance-none block w-full bg-grey-lighter text-grey-darker border  rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="grid-image" type="text" name="image" value="{{ $item->image }}" placeholder="https://via.placeholder.com/500x350">
    </div>
    <div class="w-full md:w-1/2 px-3">
      <label class="block uppercase tracking-wide text-grey-darker text-xs font-bold mb-2" for="grid-reading_time">
        Reading time
      </label>
      <input class="appearance-none block w-full bg-grey-lighter text-grey-darker border border-grey-lighter rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-grey" id="grid-reading_time" name="reading_time" type="text" value="{{$item->reading_time}}" placeholder="5 min">
    </div>
  </div>
  ...
  <div class="flex flex-wrap -mx-3 mb-6">
    <div class="w-full px-3">
      ...
      <textarea id="grid-description" name="description" class="appearance-none block w-full bg-grey-lighter text-grey-darker border  rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" placeholder="My awesome article description." rows="6">{{$item->description}}</textarea>
    </div>
  </div>
    ...
    <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
      ...
      <div class="relative">
        <select class="block appearance-none w-full bg-grey-lighter border border-grey-lighter text-grey-darker py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-grey" id="grid-is_popular" name="is_popular">
            <option value="1" {{ $item->is_popular==1 ? 'selected' : '' }}>Yes</option>
            <option value="0" {{ $item->is_popular==0 ? 'selected' : '' }}>No</option>
        </select>
        <div class="pointer-events-none absolute pin-y pin-r flex items-center px-2 text-grey-darker">
          <svg class="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/></svg>
        </div>
